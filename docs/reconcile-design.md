...
| **Offline / NotReady nodes** | Nodes whose `Ready` condition ≠ True are **excluded from the actionable target set** (not counted in updatedNodes/pending). Surfaced (v0.3) in `status.skippedNodes` + `Degraded=True(NodesSkipped)`; a rollout can reach Done with skipped nodes (Ready=True + Degraded). When a node returns, the Node watch re-triggers and it slots back into its plan wave. This keeps one dead edge box from blocking the fleet, visibly. |
| **Stuck terminating pod** (deleted but node can't confirm) | Surfaced (v0.3): a pod Terminating past `defaultStuckThreshold` (5m, overridable) sets `Degraded=True(PodStuckTerminating)` with the node names. Still no force-delete (dangerous on edge) — the operator intervenes; the wedge is now visible instead of silent. Configurable `stuckTimeoutSeconds` + skip budget still to come. |
