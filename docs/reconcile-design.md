...
* First deployment: the DS schedules the image to **all** target nodes at once. For a *new* rollout onto empty nodes this bypasses waves. Accepted for MVP (see §9); mitigation later is creating the DS with the previous image first, or gating initial scheduling. For the primary use case — *upgrading* an already-running fleet agent — waves work exactly as intended.
...
* Multiple containers / full pod template in spec (MVP assumes single-container template generated by the controller).
