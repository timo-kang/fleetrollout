...
| Item | Fixes | Status |
|---|---|---|
| `spec.template` (full PodTemplateSpec) + rolled-image field; updatedness via pod-template-generation | C5 | ✅ `spec.image` XOR `spec.template` (image is shorthand for a single `agent` container). The rolled artifact's identity is a **template hash** (any field change — env/resources/volumes — re-rolls), not the image string; `updated()` = hash-label match + scheduled + Ready. `status.lastGood` stores the rendered base template so rollback can re-render. fake-client unit tests + e2e S6 (full template with env/resources → Done). |
...
| Gate auth/TLS (`secretRef`, CA bundle) + operators/thresholds + per-wave query templating | H2/H3 | 🔄 **A (H3 ops/multi-query) + B (H2 auth/TLS) done.** A: `healthGate.queries[]` with per-query `op` (gt/ge/lt/le/eq/ne) + `threshold` (`resource.Quantity`), ANDed; bare `query` shorthand (CEL exactly-one-of); safety fix — empty vector / non-2xx / oversized (>1 MiB `io.LimitReader`) / parse-fail → *no-data → HOLD*, not "unhealthy". B: `healthGate.auth` (bearer/basic from a Secret, keys `token` / `username`+`password`) and `healthGate.tls` (CA from Secret/ConfigMap key `ca.crt`, `serverName`, `insecureSkipVerify`; CEL mutual-exclusion). Credential reads are **uncached** (`GetAPIReader`, get-only RBAC — no informer cache of every Secret). A config error (missing/garbled secret, bad CA) surfaces as `Degraded`/`HealthGateConfigValid=False` and **holds** — it never reaches decideGate, so it can't be laundered into a rollback (sabotage-verified). Unit + envtest CEL + e2e 8/8. **C (per-wave templating) done:** gate queries are Go `text/template`s over `{{.WaveNodes}}` (regex alternation of the wave's node names, `regexp.QuoteMeta`-escaped), `{{.Wave}}`, `{{.Image}}`, `{{.TemplateHash}}`, rendered before URL-escape so a wave's gate measures only its own nodes (not the diluted whole fleet). A malformed template / unknown var → `Degraded` (config error, holds — never rolls back). Unit + sabotage + controller-level render test. **H2/H3 complete.** |
...
| GHCR multi-arch (amd64+arm64) images, tagged releases, published Helm chart | REL1/REL2 | ✅ `release.yml` on `v*` tags: buildx multi-arch image → `ghcr.io/…/fleetrollout:vX.Y.Z` + `:latest`; OCI Helm chart (image pinned to tag) → `oci://ghcr.io/…/charts/fleetrollout`; GitHub Release with a consolidated `install.yaml` (server-side apply) + chart `.tgz` + generated notes. README install section + `docs/RELEASING.md` (incl. one-time GHCR public-visibility step). Chart package flow validated locally (helm lint/package/template + build-installer). Publish happens on first tag push. |
| Stuck-pod detection condition + `skippedNodes` surfacing | C7/C8 | ✅ NotReady planned nodes → `status.skippedNodes` (+ `Skipped` printer column) and `Degraded=True(NodesSkipped)`; a rollout can reach Done with skipped nodes (Ready=True + Degraded). Pods wedged Terminating past a threshold (`defaultStuckThreshold` 5m, overridable) → `Degraded=True(PodStuckTerminating)` with node names — **never force-deleted** (unsafe on edge nodes). Unit + sabotage tests; happy-path (Degraded=AllNodesReady) verified live. Configurable `stuckTimeoutSeconds` spec field + skip budget deferred. |
| Node-event filtering; pod cache label-scoping | S1/S3 | ✅ **S1**: a `nodePredicate` drops heartbeat-only Node updates (enqueue only on label / Ready-condition change; create/delete still enqueue) — kills the reconcile storm from per-second Node writes on a flapping fleet. **S3**: `ScopedCacheOptions` restricts the Pod informer to owner-labeled pods (`cache.Options.ByObject`) — large memory saving on big clusters. Unit + sabotage tests; rollout + upgrade verified live under the scoped cache. Residual (documented): the node map-func still can't map a node that *stops* matching the selector. |
