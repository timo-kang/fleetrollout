...
| Gate auth/TLS (`secretRef`, CA bundle) + operators/thresholds + per-wave query templating | H2/H3 | 🔄 **H3 operators/multi-query (increment A) done:** `healthGate.queries[]` with per-query `op` (gt/ge/lt/le/eq/ne) + `threshold` (`resource.Quantity`), all ANDed; bare `query` kept as shorthand (CEL: exactly-one-of). **Safety fix:** empty vector / non-2xx / oversized (>1 MiB, `io.LimitReader`) / parse-fail now map to *no-data → HOLD*, not "unhealthy" — closes a latent rollback-on-empty-metric hazard. Unit + sabotage + envtest CEL + e2e 8/8. **Remaining: auth/TLS (secretRef, CA) — increment B; per-wave templating — increment C.** |
